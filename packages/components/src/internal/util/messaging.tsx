...
export function resolveErrorMessage(
    ...
    if (returnInitialMsg) {
        ...
        } else if (errorMsg.indexOf('Invalid SampleSet name.') >= 0 || errorMsg.indexOf('Invalid DataClass name.') >= 0) {
            errorMsg = errorMsg.replace("Domain", errorMsg.indexOf('Invalid SampleSet name.') >= 0 ? 'Sample type' : 'Source type');
            errorMsg = errorMsg.replace('Invalid SampleSet name.', 'Invalid sample type name.');
            return errorMsg.replace('Invalid DataClass name.', 'Invalid source type name.');
        } else if (errorMsg.indexOf('ERROR: invalid byte sequence for encoding') > -1) {
            if (errorMsg.indexOf('0x00') > 0)
                return verbPresent === 'import' ? "Import file contains unsupported 'NULL' characters." : "'NULL' is an unsupported character.";
            return verbPresent === 'import' ? "Import file contains unsupported characters." : "Unsupported characters detected.";
        }
    }
    return errorMsg;
}
